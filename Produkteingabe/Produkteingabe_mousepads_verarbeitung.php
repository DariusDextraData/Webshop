...
//________________________________________________________________________________________________________________________________________________________________
    ...
    $Abfrage = "SELECT MAX(ArtikelNr) AS MaxArtikelNr FROM mousepads";

    $result = mysqli_query($connect, $Abfrage); /*$result = Abfrage wird an die DB gesendet*/

    $MousepadsAdd = "INSERT INTO mousepads VALUES('$ArtikelNr', '$Hersteller', '$Produktname', '$Einzelpreis', '$Abmessungen', '$Material');";
...
//________________________________________________________________________________________________________________________________________________________________
        ...
        if(isset($_POST['Send']))
        {
            ...
            mysqli_query($connect, $MousepadsAdd) or die (mysqli_error($connect));
            }
                else {
                /*Wenn keine Nr in der Tabelle gefunden wurde wird diese neu gesetzt.*/
                $ArtikelNr = 1025000001;
                mysqli_query($connect, $MousepadsAdd) or die (mysqli_error($connect));
                }
        }
